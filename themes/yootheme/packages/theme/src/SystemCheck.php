...
abstract class SystemCheck
{
    ...
    public function getRequirements()
    {
        ...
        if (!extension_loaded('JSON')) {
            $res[] = trans(
                '<code>json_encode()</code> must be available. Install and enable the <a href="http://php.net/manual/en/book.json.php" target="_blank">JSON</a> extension.',
            );
        }
        ...
        if (
            ...
            $res[] = trans(
                'No image library is available to process images. Install and enable the <a href="http://php.net/manual/en/book.image.php" target="_blank">GD</a> extension.',
            );
        }

        if (!extension_loaded('mbstring')) {
            $res[] = trans(
                'Multibyte encoded strings such as UTF-8 can\'t be processed. Install and enable the <a href="http://php.net/manual/en/book.mbstring.php" target="_blank">Multibyte String extension</a>.',
            );
        }
    ...
    public function getRecommendations()
    {
        ...
        if (version_compare('8.1', PHP_VERSION, '>')) {
            $res[] = trans(
                'The current PHP version %version% is outdated. Upgrade the installation, preferably to the <a href="http://php.net" target="_blank">latest PHP version</a>.',
                ['%version%' => PHP_VERSION],
            );
        }
        ...
        if (extension_loaded('GD') && !is_callable('imagewebp')) {
            $res[] = trans(
                'WebP image format isn\'t supported. Enable WebP support in the <a href="http://php.net/manual/en/image.installation.php" target="_blank">GD extension</a>.',
            );
        }
        ...
        if ($post_max_size < $this->parseSize('8M')) {
            $res[] = trans(
                'A higher upload limit is recommended. Set the <code>post_max_size</code> to 8M in the <a href="http://php.net/manual/en/ini.core.php" target="_blank">PHP configuration</a>.',
            );
        }
        ...
        if ($upload_max_filesize < $this->parseSize('8M')) {
            $res[] = trans(
                'A higher upload limit is recommended. Set the <code>upload_max_filesize</code> to 8M in the <a href="http://php.net/manual/en/ini.core.php" target="_blank">PHP configuration</a>.',
            );
        }
        ...
        if ($memory_limit > 0 && $memory_limit < $this->parseSize('128M')) {
            $res[] = trans(
                'A higher memory limit is recommended. Set the <code>memory_limit</code> to 128M in the <a href="http://php.net/manual/en/ini.core.php" target="_blank">PHP configuration</a>.',
            );
        }
